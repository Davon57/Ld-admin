...
**功能描述**：管理员/版主重置指定用户密码；重置后目标用户会被标记为下次登录必须改密。

**接口路由**：`POST /users/reset-password`
...
- `userId`（string，必填）：用户业务 ID（格式：LD####AAAA）
- `password`（string，必填）：新密码（长度 6-100）
...
| code | string | 状态码 | '0' |
| message | string | 状态描述 | 'success' |
| data | object | 数据（ResetPasswordResult） | - |

`data` 字段结构（ResetPasswordResult）：
...
**功能描述**：返回管理端首页所需的核心指标卡片、趋势图、结构图、事项追踪与（可选）列表数据。仅 admin/moderator 可用。

**接口路由**：`POST /dashboards/home`
...
- `rangeType`（string，可选）：时间范围类型（7d/14d/30d/12w），默认 14d
- `dateStart`（string，可选）：起始日期（ISO8601 / YYYY-MM-DD）；传入后优先使用自定义范围
- `dateEnd`（string，可选）：结束日期（ISO8601 / YYYY-MM-DD）；默认当天
- `granularity`（string，可选）：图表粒度（day/week）；默认随 rangeType（12w 默认 week，其余默认 day）
- `timezone`（string，可选）：时区标识（默认 Asia/Shanghai）
- `cashflowMonth`（string，可选）：记账月（YYYY-MM），默认当月
- `includeLists`（boolean，可选）：是否返回列表数据（热帖/反馈），默认 true
- `listSize`（number，可选）：列表条数（1~20），默认 10
...
| code | string | 状态码 | '0' |
| message | string | 状态描述 | 'success' |
| data | object | 数据（DashboardHomeResult） | - |

`data` 字段结构（DashboardHomeResult）：

| 字段 | 类型 | 说明 | 示例 |
| --- | --- | --- | --- |
| data.meta | object | 元信息 | - |
| data.meta.rangeType | string | 区间类型（custom/7d/14d/30d/12w） | 14d |
| data.meta.rangeStart | string | 区间起始日期（YYYY-MM-DD） | 2026-02-01 |
| data.meta.rangeEnd | string | 区间结束日期（YYYY-MM-DD） | 2026-02-14 |
| data.meta.granularity | string | 图表粒度（day/week） | day |
| data.meta.timezone | string | 时区 | Asia/Shanghai |
| data.meta.updatedAt | string | 刷新时间（YYYY-MM-DD HH:mm:ss） | 2026-02-03 12:00:00 |
| data.cards | array | 指标卡片数组 | - |
| data.cards[].key | string | 指标 key（如 memberTotal/articleTotal/qaQuestionTotal/articleHotTotal/qaFeaturedTotal/goodThingTotal/goodThingBoughtTotal/goodThingSelfTotal 等） | memberTotal |
| data.cards[].title | string | 标题 | 总成员数 |
| data.cards[].value | number | 数值 | 1200 |
| data.cards[].unit | string | 单位 | 人 |
| data.accounting | object | 记账模块 | - |
| data.accounting.month | string | 记账月（YYYY-MM） | 2026-02 |
| data.accounting.cards | array | 记账卡片数组 | - |
| data.charts | object | 图表数据 | - |
| data.charts.newMemberTrend | object | 新成员趋势 | - |
| data.charts.newMemberTrend.xAxis | array | X 轴（日期或周起始日） | ['2026-02-01'] |
| data.charts.newMemberTrend.series | array | series | - |
| data.charts.*.series[].type | string | ECharts series 类型（line/bar/pie 等） | line |
| data.charts.*.series[].name | string | ECharts series 名称 | 新增成员 |
| data.charts.*.series[].data | array | ECharts series 数据 | [12, 18] |
| data.charts.interactionTrend | object | 互动趋势（提问 vs 解答） | - |
| data.charts.contentValueStructure | object | 内容价值结构 | - |
| data.charts.contentValueStructure.explain | string | 口径说明（统计范围与分类规则） | 统计范围：全量（不受 meta.rangeStart/meta.rangeEnd 影响）。categories 与 values 一一对应。普通问答：非精选且不热门；热门问答：回答数>=10且点赞数>5；精选问答：isFeatured=true；优质问答：同时满足精选与热门。 |
| data.charts.contentValueStructure.categories | array | 分类（普通问答/热门问答/精选问答/优质问答） | ['普通问答', '热门问答', '精选问答', '优质问答'] |
| data.charts.contentValueStructure.values | array | 各分类数量（与 categories 对齐） | [120, 18, 6, 3] |
| data.charts.contentValueStructure.option | object | ECharts option（bar + pie 组合） | - |
| data.charts.contributionPie | object | 用户角色结构 | - |
| data.charts.contributionPie.option | object | ECharts option（pie） | - |
| data.charts.goodThingPublishTypePie | object | 好物类型关系（买过 vs 自出） | - |
| data.charts.goodThingPublishTypePie.option | object | ECharts option（pie） | - |
| data.charts.accountingSummary | object | 记账汇总（收入/支出/净额） | - |
| data.charts.accountingSummary.option | object | ECharts option（bar） | - |
| data.tracking | object | 事项追踪 | - |
| data.tracking.firstContributionAvgDays | number | 新成员首次贡献平均耗时（天） | 2.4 |
| data.tracking.issue | object | 内容问题追踪 | - |
| data.tracking.issue.pendingCount | number | 待解决问题数（未采纳） | 30 |
| data.tracking.issue.avgResolveHours | number | 平均解决耗时（小时） | 9.6 |
| data.tracking.issue.resolveRate | number | 解决率（%） | 50 |
| data.lists | object | 列表数据（includeLists=false 时 list 为空） | - |
| data.lists.hotContent | object | 热帖列表 | - |
| data.lists.hotContent.list | array | 热帖数组 | - |
| data.lists.hotContent.list[].title | string | 标题 | 轮胎怎么选？ |
| data.lists.hotContent.list[].createdAt | string \ null | 时间（YYYY-MM-DD HH:mm:ss） | 2026-02-03 12:00:00 |
| data.lists.hotContent.list[].metric | number | 指标值（默认热度） | 123 |
| data.lists.latestFeedback | object | 最新反馈列表 | - |
| data.lists.latestFeedback.list | array | 反馈数组 | - |
| data.lists.latestFeedback.list[].title | string | 标题（优先 type） | bug |
| data.lists.latestFeedback.list[].content | string | 内容 | ... |
| data.lists.latestFeedback.list[].summary | string | 摘要 | ... |
| data.lists.latestFeedback.list[].createdAt | string \ null | 创建时间 | 2026-02-03 12:00:00 |
| data.lists.latestFeedback.list[].updatedAt | string \ null | 更新时间 | 2026-02-03 12:10:00 |
...
**功能描述**：直接从七牛空间列举指定前缀下的文件；使用 marker 游标分页；可选返回 url / downloadUrl。普通用户会强制使用默认空间，且前缀固定为环境变量 QINIU_WALLPAPER_PREFIX（未配置时为 ''）。

**接口路由**：`POST /car-wallpapers/qiniu-list`
...
- `prefix`（string，选填，默认 car-wallpapers/）：前缀筛选
- `marker`（string，选填，默认 ''）：分页游标；传上一次返回的 nextMarker
- `limit`（number，选填，默认 100）：每次返回数量（1-1000）
- `delimiter`（string，选填，默认 ''）：目录分隔符（常用 '/'）
- `qiniuBucket`（string，选填）：七牛空间（默认取环境变量 QINIU_BUCKET）
- `qiniuDomain`（string，选填）：访问域名（默认取环境变量 QINIU_DOMAIN；仅当 withUrls=true 生效）
- `isQiniuPrivate`（boolean，选填）：是否私有（默认取环境变量 QINIU_IS_PRIVATE；仅当 withUrls=true 时影响 downloadUrl）
- `withUrls`（boolean，选填，默认 true）：是否返回 url/downloadUrl 字段
...
| code | string | 状态码 | '0' |
| message | string | 状态描述 | 'success' |
| data | object | 数据 | { items: [], nextMarker: '', commonPrefixes: [] } |
...
| --- | --- | --- | --- |
| items | array | 文件列表 | - |
| nextMarker | string | 下一页游标（为空表示结束） | l2... |
| commonPrefixes | array | 子目录前缀（delimiter 生效时可能返回） | ["car-wallpapers/2026/"] |
...
| qiniuDomain | string \ null | 访问域名（withUrls=false 时为 null） | https://cdn.example.com |
| isQiniuPrivate | boolean | 是否私有 | false |
| hash | string \ null | 七牛 hash | Ft... |
| sizeBytes | number \ null | 文件大小（字节） | 123456 |
| mimeType | string \ null | MIME 类型 | image/jpeg |
| putTime | number \ string \ null | 上传时间（七牛返回） | 17387328000000000 |
| url | string \ null | 公网访问 URL（withUrls=true 时有值） | https://cdn.example.com/car-wallpapers/xxx.jpg |
| downloadUrl | string \ null | 私有下载 URL（私有且 withUrls=true 时有值） | https://...&token=... |
| downloadUrlExpiredAt | string \ null | 私有链接过期时间 | 2026-02-05 13:00:00 |
